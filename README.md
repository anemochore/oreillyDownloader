...
6. 2021년 러닝 오라일리 사이트 개편(레이지 로딩)으로 업데이트 중단.
7. 22-5-6 탬퍼멍키 스크립트 방식으로 jpub.kr 사용자에게만 재오픈. 일단 매닝만 지원.

## 사용법(v0.1 이후)
1. 탬퍼멍키 및 탬퍼멍키 스크립트(드롭박스에 호스팅)를 설치한다.
2. 러닝 오라일리 로그인 후 도서 메인 페이지(https://learning.oreilly.com/library/view/TITLE/ISBN/ 같은 주소)에서 '다운로드 시작' 버튼 클릭.
3. 자동으로 챕터를 끝까지 탐색하며 스크레이핑 후, 리포매팅한 파일 등을 담은 zip 다운로드가 시작됨.
...
- ...
- 2025-11-07 탬퍼멍키 스크립트만으로 작동하게 변경(북마클릿 삭제)

## todo
1. ~~오라일리 외 출판사 책도 가져오게 해보면 어떨까? -> 현재 9개 출판사 (부분)지원~~
2. ~~목차 클릭 시 해당 요소 하이라이트~~ -> CSS로 해결
3. 이미지 클릭 시 확대가 되면 좋을 듯.
4. ~~오라일리 팁 박스 등에 텍스트 넣기~~ -> 해결?
5. ~~css 분리~~ -> 대충 완료